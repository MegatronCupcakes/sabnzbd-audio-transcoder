...
const transcode = (request) => {
    return new Promise((resolve, reject) => {
        try {
            const successBool = _transcode(request);
            resolve(successBool);
        } catch(error){
            reject(error);
        }
...
const _transcode = (request) => {
    return new Promise(async (resolve, reject) => {
        try {
            let albumDirectory = request.testMode ? request.body.path : path.join(downloadsDir, _.last(request.body.path.split("/")));
            // fix permissions
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (const file of files){
                let _fileParts = file.split(".");
                _fileParts.pop();
                let outputFile = `${_fileParts.join(".")}.${codec}`;
